...
class PolicyNetTrpo(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim):
        super(PolicyNetTrpo, self).__init__()
...
class ValueNetTrop(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim):
        super(ValueNetTrop, self).__init__()
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim)
...
class TRPO:
    def __init__(self, hidden_dim, state_space, action_space, lmbda, kl_constraint, alpha, critic_lr, gamma, device):
        ...
        # 策略网络参数不需要优化器更新
        self.actor = PolicyNetTrpo(state_dim, hidden_dim, action_dim).to(device)
        self.critic = ValueNetTrop(state_dim, hidden_dim).to(device)
